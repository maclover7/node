...
async function readFileHandle(filehandle, options) {
  ...
  const statFields = await binding.fstat(filehandle.fd, false, req);
...
async function ftruncate(handle, len = 0) {
  ...
  len = Math.max(0, len);
  const req = new FSReqPromise();
  return binding.ftruncate(handle.fd, len, req);
}
...
async function rmdir(path) {
  ...
  validatePath(path);
  const req = new FSReqPromise();
  return binding.rmdir(pathModule.toNamespacedPath(path), req);
}

async function fdatasync(handle) {
  validateFileHandle(handle);
  const req = new FSReqPromise();
  return binding.fdatasync(handle.fd, req);
}

async function fsync(handle) {
  validateFileHandle(handle);
  const req = new FSReqPromise();
  return binding.fsync(handle.fd, req);
}
...
async function mkdir(path, mode) {
  ...
  const req = new FSReqPromise();
  return binding.mkdir(pathModule.toNamespacedPath(path), mode, req);
}
...
async function fstat(handle, options = { bigint: false }) {
  validateFileHandle(handle);
  const req = new FSReqPromise(options.bigint);
  const result = await binding.fstat(handle.fd, options.bigint, req);
...
async function unlink(path) {
  ...
  validatePath(path);
  const req = new FSReqPromise();
  return binding.unlink(pathModule.toNamespacedPath(path), req);
}
...
async function fchmod(handle, mode) {
  ...
  const req = new FSReqPromise();
  return binding.fchmod(handle.fd, mode, req);
}
...
async function chmod(path, mode) {
  ...
  mode = validateMode(mode, 'mode');
  const req = new FSReqPromise();
  return binding.chmod(pathModule.toNamespacedPath(path), mode, req);
}
...
async function fchown(handle, uid, gid) {
  ...
  validateUint32(gid, 'gid');
  const req = new FSReqPromise();
  return binding.fchown(handle.fd, uid, gid, req);
}
...
async function futimes(handle, atime, mtime) {
  ...
  const req = new FSReqPromise();
  return binding.futimes(handle.fd, atime, mtime, req);
}
...
async function mkdtemp(prefix, options) {
  ...
  nullCheck(prefix);
  const req = new FSReqPromise();
  return binding.mkdtemp(`${prefix}XXXXXX`, options.encoding, req);
}
